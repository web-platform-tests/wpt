 ...
 *
 * You can also test that particular objects implement an interface with
 *   interface_obj.test_primary_interface_of(x, "description of x");
 * where interface_obj is an IdlInterface and x is some object.  For instance:
 *   interface_obj.test_primary_interface_of(document.body, "document.body");
 * This currently only supports testing that an object implements the interface
 * as its primary interface, not any interfaces further up the chain.
 *
 * In the future, more tests will hopefully be autogenerated, and more methods
 * exposed.
 */
"use strict";
(function(){
...
IdlInterface.prototype.test_primary_interface_of = function(obj, desc)
{
    var interface_obj = this;
    ...
    if (!this.has_extended_attribute("NoInterfaceObject"))
    {
        test(function() {
            ...
            assert_true(window[interface_obj.name].prototype.isPrototypeOf(obj),
                desc + "'s prototype is not " + interface_obj.name + ".prototype");

        }, desc + " must implement " + this.name);
    }
    ...
    for (var i = 0; i < interface_obj.members.length; i++)
    {
        var member = interface_obj.members[i];
        if (member.has_extended_attribute("Unforgeable"))
        {
            test(function() {
                assert_own_property(obj, member.name);
            }, desc + " must have own property " + member.name);
        }
        ...
        && member.name)
        {
            ...
            }, desc + " must inherit property " + member.name + " with the proper type");
        }
        ...
        if (member.type == "operation" && member.name && member.arguments.length)
        {
            ...
            }, "Calling " + member.name + "() on " + desc + " with too few arguments must throw TypeError");
        }
    }
}
