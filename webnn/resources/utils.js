...
/**
 * Helper to construct a typed array from array buffer with data.
 ...
 * @param {String} [type="uint8"] - The data type of the typed array to return.
 * @return {TypedArray} - Array view to return of type.
 */
const create_array_view = (array_buffer, array_data, type = 'uint8') => {
  let typed_array = new TypedArrayDict[type](array_buffer);
  typed_array.set(array_data);
  return typed_array;
}
...
const testWriteWebNNBuffer = (testName, deviceType = 'cpu') => {
  ...
  promise_test(async () => {
    ...
    assert_throws_js(TypeError, () => ml_context.writeBuffer(ml_buffer, new Uint8Array(array_buffer), /*srcOffset=*/0, /*srcSize=*/ml_buffer.size + 1));
    ...
    assert_throws_js(TypeError, () => ml_context.writeBuffer(ml_buffer, new Uint8Array(array_buffer), /*srcOffset=*/ml_buffer.size + 1));

    // Writing with a source offset that is out of range of implicit copy size.
    assert_throws_js(TypeError, () => ml_context.writeBuffer(ml_buffer, new Uint8Array(array_buffer), /*srcOffset=*/ml_buffer.size + 1, /*srcSize=*/undefined));

    // Writing with a source offset of undefined should be treated as 0.
    ml_context.writeBuffer(ml_buffer, new Uint8Array(array_buffer), /*srcOffset=*/undefined, /*srcSize=*/ml_buffer.size);

    assert_throws_js(TypeError, () => ml_context.writeBuffer(ml_buffer, new Uint8Array(array_buffer), /*srcOffset=*/undefined, /*srcSize=*/ml_buffer.size + 1));
...
const testReadWebNNBuffer = (testName, deviceType = 'cpu') => {
  ...
  promise_test(async () => {
    ...
    ml_context.writeBuffer(ml_buffer, create_array_view(array_buffer, [0xBBBBBBBB], 'uint32'));
    await assert_buffer_data_equals(ml_context, ml_buffer, Uint32Array.from([0xBBBBBBBB]));;
  ...
  promise_test(async () => {
    ...
    // Initialize the buffer.
    ml_context.writeBuffer(ml_buffer, create_array_view(new ArrayBuffer(ml_buffer.size), [0xAA, 0xAA, 0xAA, 0xAA]));

    // Writing to the remainder of the buffer from source offset.
    ml_context.writeBuffer(ml_buffer, create_array_view(new ArrayBuffer(ml_buffer.size), [0xAA, 0xAA, 0xBB, 0xBB]), /*srcOffset=*/2);
    await assert_buffer_data_equals(ml_context, ml_buffer, Uint8Array.from([0xBB, 0xBB, 0xAA, 0xAA]));
  ...
  promise_test(async () => {
    ...
    await assert_buffer_data_equals(ml_context, ml_buffer, Uint8Array.from(input_data));
  ...
  promise_test(async () => {
    ...
    ml_context.writeBuffer(ml_buffer, create_array_view(new ArrayBuffer(ml_buffer.size), [0xDD, 0xDD, 0xCC, 0xDD]), /*srcOffset=*/2, /*srcSize=*/1);
    await assert_buffer_data_equals(ml_context, ml_buffer, Uint8Array.from([0xCC, 0xAA, 0xAA, 0xAA]));
  ...
  promise_test(async () => {
    ...
    // Writing from a larger source buffer that allows it to fit in the buffer when using a offset.
    ml_context.writeBuffer(ml_buffer, Uint8Array.from([0xEE, 0xEE, 0xEE, 0xEE, 0xEE]), /*srcOffset=*/1);
    await assert_buffer_data_equals(ml_context, ml_buffer, Uint8Array.from([0xEE, 0xEE, 0xEE, 0xEE]));
