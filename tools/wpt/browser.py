...
def get_firefox_git_tags(ref_prefix: str) -> List[str]:
    tags = []
    for tag_data in get(
...
class FirefoxPrefs:
    ...
    def install_prefs(self, binary: Optional[str], dest: Optional[str] = None, channel: Optional[str] = None):
    ...
    def get_git_ref(self, version: Optional[str], channel: str, rev: Optional[str]) -> str:
        ...
        for tag in get_firefox_git_tags(ref_prefix):
            m = ref_re.match(tag)
            if not m:
                continue
