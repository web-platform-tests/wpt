(function DOMParsingAndSerializationPF(global) {
   ...
   function serializeElement(node, namespace, prefixMap, prefixIndexRef, requireWellFormed) {
      ...
      //   keys, with a corresponding namespace as the key's value. Note: This map is local to each element.
      ...
      //    namespace of its parent). Run these sub-steps:
         ...
         // (FIXED)4.Otherwise, if prefix is not null, then:
            ...
            if (localPrefixesMap[prefix])
               prefix = generateAPrefix(map, ns, prefixIndexRef);
            // 2.(EDITED)Otherwise, 'add' 'prefix' to map given namespace 'ns'
            else
               map.add(ns, prefix);
   ...
   function recordTheNamespaceInfo(element, map, localPrefixesMap) {
      ...
      [].slice.call(element.attributes).forEach(function recordTheNamespaceInfoMainLoop(attr) {
         ...
         if (attributeNamespace == XMLNSNS) {
            ...
            // 2.75.(NEW)If namespace definition is the XML namespace, then stop running these steps,
            ...
            // 6.Add the value of prefix definition as a new key to the local prefixes map, with the namespace definition
            //   as the key's value.
      ...
      [].slice.call(element.attributes).forEach(function xmlSerializationOfNodeAttributesMainLoop(attr) {
         ...
         if (attributeNamespace != null) {
            ...
            if (attributeNamespace == XMLNSNS) {
               ...
               //    * the attr's prefix is not null and either the attr's localName is not a key in the local prefixes map, or it
               ...
               //      (the current namespace prefix definition was exactly defined previously--on an ancestor element not the current one);
               if ((attr.value == XMLNS) ||
                   ((attr.prefix == null) && ignoreNamespaceDefinition) ||
                   ((attr.prefix != null) && 
                      (!localPrefixesMap[attr.localName] || (localPrefixesMap[attr.localName] != attr.value)) && 
