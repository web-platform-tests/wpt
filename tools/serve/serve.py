...
def check_subdomains(host, paths, bind_hostname, ssl_config, aliases):
    ...
    if not connected:
        logger.critical("Failed to connect to test server on http://%s:%s. You may need to edit /etc/hosts or similar, see README.md." % (host, port))
        sys.exit(1)
    ...
    for subdomain, (punycode, host) in subdomains.iteritems():
        ...
        try:
            ...
            logger.critical("Failed probing domain %s. You may need to edit /etc/hosts or similar, see README.md." % domain)
            sys.exit(1)
