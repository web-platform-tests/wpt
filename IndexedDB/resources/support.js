...
var DB3Factory = window.db3Js.DB3Factory
var generateKey = window.db3Js.generateKey
var sign = window.db3Js.sign
...
window.indexedDB3 = new DB3Factory({
...
add_completion_callback(function (tests) {
    for (var i in tests) {
        if (tests[i].db) {
            tests[i].db.close()
            indexedDB3.deleteDatabase(tests[i].db.name)
        }
    }
})
...
function createdb_for_multiple_tests(dbname, version) {
    ...
    if (version) rq_open = indexedDB3.open(dbname, version)
    else rq_open = indexedDB3.open(dbname)
...
function assert_key_equals(actual, expected, description) {
    assert_equals(indexedDB3.cmp(actual, expected), 0, description)
}
...
function indexeddb_test(upgrade_func, open_func, description, options) {
    async_test(function (t) {
        ...
        var del = indexedDB3.deleteDatabase(dbname)
        del.onerror = t.unreached_func('deleteDatabase should succeed')
        var open = indexedDB3.open(dbname, 1)
        open.onupgradeneeded = t.step_func(function () {
            ...
            t.add_cleanup(function () {
                ...
                db.close()
                indexedDB3.deleteDatabase(db.name)
            })
...
async function createIndexedDBForTesting(rc, dbName, version) {
    await rc.executeScript(
        (dbName, version) => {
            let request = indexedDB3.open(dbName, version)
