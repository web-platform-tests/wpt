...
def get_paths(dest):
    ...
    paths = {
        "base": base_path,
        "sdk": os.environ.get("ANDROID_SDK_HOME", os.path.join(base_path, f"android-sdk-{os_name}")),
    }
    paths["avd"] = os.environ.get("ANDROID_AVD_HOME", os.path.join(paths["sdk"], ".android", "avd"))
    paths["sdk_tools"] = os.path.join(paths["sdk"], "cmdline-tools", CMDLINE_TOOLS_VERSION_STRING)
    paths["emulator_home"] = os.path.dirname(paths["avd"])
    return paths


def get_sdk_manager(paths):
    os_name = platform.system().lower()
...
def install_android_packages(logger, paths, packages, prompt=True):
    sdk_manager = get_sdk_manager(paths)
