...
class RefTestImplementation(object):
    ...
    def get_hash(self, test, viewport_size, dpi):
        ...
        if key not in self.screenshot_cache:
            ...
            screenshot = base64.decodestring(data)
    ...
    def get_differences(self, screenshots):
        lhs = Image.open(io.BytesIO(screenshots[0])).convert("RGB")
        rhs = Image.open(io.BytesIO(screenshots[1])).convert("RGB")
    ...
    def run_test(self, test):
        ...
        log_data = [{"url": nodes[0].url, "screenshot": base64.encodestring(screenshots[0])}, relation,
                    {"url": nodes[1].url, "screenshot": base64.encodestring(screenshots[1])}]
