...
function startWorkerAndObserveReports(worker_url, wait_for_report) {
  ...
  const result_promise = new Promise(resolve => {
    worker.port.onmessage = _ => resolve('success');
    worker.onerror = _ => resolve('error');
  });
  worker.port.postMessage("postMessage('reply to owner from worker');");
  ...
  if (wait_for_report) {
    Promise.all([result_promise, report_promise]).then(results => {
      parent_or_self.port.postMessage(results[1]);
    });
  } else {
    result_promise.then(result => {
      parent_or_self.port.postMessage([]);
    });
  }
}
