...
idl_test(
  ...
  async (idl_array) => {
    ...
    for (const executionType of ExecutionArray) {
      ...
      self.input = builder.input('input', {dataType: 'float32', type: 'float32', dimensions: [1, 1, 5, 5]});
      self.filter = builder.constant({dataType: 'float32', type: 'float32', dimensions: [1, 1, 3, 3]}, new Float32Array(9).fill(1));
