...
@pytest.mark.asyncio
async def test_find_by_css(bidi_session, inline, top_context):
...
@pytest.mark.asyncio
async def test_find_by_css_limit_return_count(bidi_session, inline, top_context):
    url = inline("""<div data-class="one">One</div><div data-class="two">Two</div>""")
    await bidi_session.browsing_context.navigate(
        context=top_context["context"], url=url, wait="complete"
    )
...
@pytest.mark.asyncio
async def test_find_by_css_with_context_nodes(bidi_session, inline, top_context):
