...
function writePersistantData($data)
{
    ...
    {
        $data .= "|" . time();
        file_put_contents("preflightData/$RequestID.txt", $data);
    }
}
...
{
    $RequestID = intval($_GET["RequestID"]);
    if($RequestID !== 0 && file_exists("preflightData/$RequestID.txt"))
    {
        $data = file_get_contents("preflightData/$RequestID.txt");
        if($data == null)
        {
            return null;
        }
        ...
        unlink("preflightData/$RequestID.txt");
