...
class Find(object):
    ...
    @command
    def css(self, element_selector, frame_selector, all=True):
        if (frame_selector != "window"):
            url = "frame"
            body = {"id": frame_selector}
            self.session.send_session_command("POST", url, body)
        return self._find_element("css selector", element_selector, all)
