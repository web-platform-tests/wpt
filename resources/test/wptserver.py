...
class WPTServer(object):
    ...
    def start(self):
        ...
        wptserveCmd = [os.path.join(self.wpt_root, 'wpt'), 'serve', '--config=' + _CONFIG_FILE]
        logging.info('Executing %s' % ' '.join(wptserveCmd))
        ...
        for retry in range(5):
            ...
            if exitCode != None:
                logging.warn('Command "%s" exited with %s', ' '.join(wptserveCmd), exitCode)
                break
            try:
