...
async def test_other_context(
    bidi_session, wait_for_event, url, top_context, setup_network_test, fetch, phase
):
...
async def test_two_intercepts(
    bidi_session, wait_for_event, url, top_context, setup_network_test, fetch
):
    network_events = await setup_network_test(
        events=[
            "network.beforeRequestSent",
    ...
    text_url = url(PAGE_EMPTY_TEXT)
    string_intercept = await bidi_session.network.add_intercept(
        phases=["beforeRequestSent"],
        url_patterns=[{"type": "string", "pattern": text_url}],
    )
    # Add a second intercept to catch all requests.
    global_intercept = await bidi_session.network.add_intercept(
        phases=["beforeRequestSent"],
        url_patterns=[{"type": "pattern"}],
    )
