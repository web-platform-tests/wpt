...
//@{
{
    ...
    if (type.sequence)
    {
        assert_true(Array.isArray(value), "is an Array");
        if (!value.length)
        {
            // Nothing we can do.
            return;
    ...
    switch(type)
    {
        ...
        case "byte":
            ...
            assert_true(-128 <= value && value <= 127, "byte " + value + " is in range [-128, 127]");
            return;

        case "octet":
            assert_equals(typeof value, "number");
            assert_equals(value, Math.floor(value), "not an integer");
            assert_true(0 <= value && value <= 255, "octet " + value + " is in range [0, 255]");
            return;

        case "short":
            assert_equals(typeof value, "number");
            assert_equals(value, Math.floor(value), "not an integer");
            assert_true(-32768 <= value && value <= 32767, "short " + value + " is in range [-32768, 32767]");
            return;

        case "unsigned short":
            assert_equals(typeof value, "number");
            assert_equals(value, Math.floor(value), "not an integer");
            assert_true(0 <= value && value <= 65535, "unsigned short " + value + " is in range [0, 65535]");
            return;

        case "long":
            assert_equals(typeof value, "number");
            assert_equals(value, Math.floor(value), "not an integer");
            assert_true(-2147483648 <= value && value <= 2147483647, "long " + value + " is in range [-2147483648, 2147483647]");
            return;

        case "unsigned long":
            assert_equals(typeof value, "number");
            assert_equals(value, Math.floor(value), "not an integer");
            assert_true(0 <= value && value <= 4294967295, "unsigned long " + value + " is in range [0, 4294967295]");
            return;
        ...
        case "DOMTimeStamp":
            assert_equals(typeof value, "number");
            assert_true(0 <= value, "unsigned long long is positive");
            return;
...
//@{
{
    test(function()
    {
        ...
        assert_true(desc.writable, "self's property " + format_value(this.name) + " is writable");
        assert_false(desc.enumerable, "self's property " + format_value(this.name) + " is enumerable");
        assert_true(desc.configurable, "self's property " + format_value(this.name) + " is configurable");
    ...
    if (!this.is_callback()) {
        test(function() {
            ...
            assert_true(desc.configurable, this.name + ".length is configurable");
    ...
    if (!this.is_callback() || this.has_constants()) {
        test(function() {
            ...
            assert_true(desc.configurable, this.name + ".name is configurable");
    ...
    test(function()
    {
        ...
        if (this.is_callback()) {
            assert_false("prototype" in self[this.name],
                         this.name + ' should have a "prototype" property');
            return;
        }
    ...
    test(function()
    {
        ...
        if (this.is_callback()) {
            assert_false("prototype" in self[this.name],
                         this.name + ' has a "prototype" property');
            return;
        }
        ...
        assert_true(desc.writable, this.name + ".prototype.constructor is writable");
        assert_false(desc.enumerable, this.name + ".prototype.constructor is enumerable");
        assert_true(desc.configurable, this.name + ".prototype.constructor is configurable");
...
//@{
{
    ...
    test(function()
    {
        ...
        assert_true(desc.enumerable, "property is enumerable");
...
//@{
{
    ...
    test(function()
    {
        ...
        assert_true(descriptor.writable, "property is writable");
        assert_true(descriptor.configurable, "property is configurable");
        ...
        if (!exposed_in(exposure_set(member, this.exposureSet))) {
            test(function() {
                ...
                assert_false(member.name in self[this.name],
                             "The interface object has a property " +
                             format_value(member.name));
                assert_false(member.name in self[this.name].prototype,
                             "The prototype object has a property " +
                             format_value(member.name));
...
//@{
{
    ...
    assert_false("value" in desc, 'property descriptor has "value" field');
    assert_false("writable" in desc, 'property descriptor has "writable" field');
    assert_true(desc.enumerable, "property is enumerable");
    if (member.isUnforgeable)
    {
        assert_false(desc.configurable, "[Unforgeable] property is configurable");
    }
    else
    {
