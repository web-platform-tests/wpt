...
def main():
    ...
    tmp_dir = os.path.join(wpt_root, "wasm", "tmp_proposals")
    ...
    try:
        for name in proposals:
            ...
            interpreter_dir = os.path.join(prop_dir, "interpreter")
            ...
            out_dir = os.path.join(prop_dir, "out")
            os.makedirs(out_dir, exist_ok=True)
            build_script = os.path.join(prop_dir, "test", "build.py")
            assert os.path.exists(build_script)
            print("Converting WAST tests to WPT format...")
            run_cmd([build_script, "--dont-recompile", "--html", out_dir], cwd=prop_dir)

            # 7. Target proposal directory in WPT
