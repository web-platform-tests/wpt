...
async def test_subscribe_unsubscribe(bidi_session, new_tab, wait_for_event, wait_for_future_safe, log_type):
    ...
    assert len(events) == 3
    assert_base_entry(events[0], text=expected_text_0, context=new_tab["context"], userContext=new_tab["userContext"])
    assert_base_entry(events[1], text=expected_text_1, context=new_tab["context"], userContext=new_tab["userContext"])
    assert_base_entry(events[2], text=expected_text_2, context=new_tab["context"], userContext=new_tab["userContext"])
    ...
    assert len(events) == 4
    assert_base_entry(events[3], text=expected_text_3, context=new_context["context"], userContext=new_tab["userContext"])
