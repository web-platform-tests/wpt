...
directory_test(async (t, root) => {
  ...
  await promise_rejects_dom(t, 'AbortError', dir.move(dir_dest));

  assert_array_equals(
      await getSortedDirectoryEntries(root), ['dir-dest/', 'dir/']);
  assert_array_equals(await getSortedDirectoryEntries(dir_dest), []);
}, 'move(dir) to move a directory fails');

directory_test(async (t, root) => {
  const dir = await root.getDirectoryHandle('dir-before', {create: true});
  const dir_dest = await root.getDirectoryHandle('dir-dest', {create: true});
  await promise_rejects_dom(t, 'AbortError', dir.move(dir_dest, 'dir-after'));

  assert_array_equals(
      await getSortedDirectoryEntries(root), ['dir-before/', 'dir-dest/']);
  assert_array_equals(await getSortedDirectoryEntries(dir_dest), []);
}, 'move(dir, name) to move a directory fails');
