...
for (const vector of vectors) {
  promise_test(async () => {
    ...
    if (vector.algorithm === "Ed25519") {
      ...
      assert_array_equals(signature, bytes(vector.signature), "independent reference signature");
      assert_true(await crypto.subtle.verify("Ed25519", publicKey, signature, message));
      return;
    }
...
for (const vector of [vectors[0], vectors[3]]) {
  for (const format of ["pkcs8", "spki"]) {
    ...
    }, `${vector.algorithm} rejects malformed ${format} around fixed-length key material`);
  }
}
