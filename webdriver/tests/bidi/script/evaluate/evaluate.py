    ...
    recursive_compare(
        {
            ...
            "exceptionDetails": {
                ...
                "exception": {"handle": any_string, "type": "error"},
                "lineNumber": any_int,
...
async def test_interact_with_dom(bidi_session, top_context):
    ...
    assert result == {"type": "string", "value": "window.location.href: about:blank"}


@pytest.mark.asyncio
async def test_resolved_promise_with_await_promise_false(bidi_session, top_context):
    result = await bidi_session.script.evaluate(
...
@pytest.mark.asyncio
async def test_resolved_promise_with_await_promise_true(bidi_session, top_context):
    result = await bidi_session.script.evaluate(
        expression="Promise.resolve('SOME_RESOLVED_RESULT')",
        target=ContextTarget(top_context["context"]),
        await_promise=True,
...
@pytest.mark.asyncio
async def test_rejected_promise_with_await_promise_false(bidi_session, top_context):
    result = await bidi_session.script.evaluate(
...
@pytest.mark.asyncio
async def test_rejected_promise_with_await_promise_true(bidi_session, top_context):
    with pytest.raises(ScriptEvaluateResultException) as exception:
        await bidi_session.script.evaluate(
            expression="Promise.reject('SOME_REJECTED_RESULT')",
            target=ContextTarget(top_context["context"]),
