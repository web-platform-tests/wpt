import pytest
from . import MAX_TOUCHES_PER_CONTEXT, MAX_TOUCHES_PER_USER_CONTEXT, MAX_TOUCHES_PER_GLOBAL
...
async def test_contexts_isolation(bidi_session, top_context,
        get_max_touch_points, initial_max_touch_points):
    ...
    assert await get_max_touch_points(another_context) == initial_max_touch_points
...
async def test_frame(bidi_session, url, get_max_touch_points,
        top_context, create_iframe, domain, initial_max_touch_points):
...
async def test_overrides_global(bidi_session, get_max_touch_points,
    ...
    await bidi_session.emulation.set_touch_override(max_touch_points=MAX_TOUCHES_PER_GLOBAL)
    assert await get_max_touch_points(affected_context) == MAX_TOUCHES_PER_CONTEXT

    await bidi_session.emulation.set_touch_override(
        max_touch_points=None,
        contexts=[affected_context["context"]])
    assert await get_max_touch_points(affected_context) == MAX_TOUCHES_PER_GLOBAL
