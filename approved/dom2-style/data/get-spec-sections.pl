...
# Modified for DOM2 Style by Peter Linss
#
...
my $baseURI = 'http://www.w3.org/TR/DOM-Level-2-Style/';
my $specURI = $baseURI . 'expanded-toc.html';
my $page = get($specURI);
$page =~ s/\x{D}\x{A}|\x{A}\{D}|\x{D}|\x{A}/\n/gos; # normalize newlines

# remove everything except the toc
$page =~ s/.*<h1 id=\'TOC-h1\'//os;
$page =~ s/href=\'references.html\'.*//os;

while ($page =~ m/<a[\s]+class=[\'\"]?tocxref[\'\"]?[\s]+href=[\'\"]?([^\"\']+)[\'\"]?>(Appendix |Chapter )?([A-Z]?[0-9.]*)[:\s]+(.+?)<\/a>/gos) {
    my $uri = "$baseURI$1";
    my $section = $3;
    my $title = $4;
    $section =~ s/\.$//gos;
