...
promise_test(async t => {
  for (let dataType of operandDataTypeArray) {
    const input = builder.input('input', {dataType, dimensions: dimensions2D});
    ...
    for (let axis of validAxisArray) {
      ...
      const mean = builder.input('mean', {dataType, dimensions: [size]});
      const variance = builder.input('variance', {dataType, dimensions: [size]});
      for (let outsideAxis of outsideAxisArray) {
        assert_throws_js(TypeError, () => builder.batchNormalization(input, mean, variance, {axis: outsideAxis}));
      }
    }
  }
...
promise_test(async t => {
  for (let dataType of operandDataTypeArray) {
    ...
    const validAxisArray = getAxisArray(dimensions2D);
    for (let axis of validAxisArray) {;
      const variance = builder.input('variance', {dataType, dimensions: [dimensions2D[axis]]});
      for (let dimensions of dimensionsArray) {
        if (dimensions.length !== 1) {
          // set mean not be 1D tensor
          const mean = builder.input('mean', {dataType, dimensions});
...
promise_test(async t => {
  for (let dataType of operandDataTypeArray) {
    ...
    for (let axis of validAxisArray) {
      let size = dimensions2D[axis];
      const variance = builder.input('variance', {dataType, dimensions: [size]});
      for (let offset of adjustOffsetArray) {
        const adjustedSize = size + offset;
        const mean = builder.input('mean', {dataType, dimensions: [adjustedSize]});
...
promise_test(async t => {
  for (let dataType of operandDataTypeArray) {
    ...
    const validAxisArray = getAxisArray(dimensions2D);
    for (let axis of validAxisArray) {;
      const mean = builder.input('mean', {dataType, dimensions: [dimensions2D[axis]]});
      for (let dimensions of dimensionsArray) {
        if (dimensions.length !== 1) {
          // set variance not be 1D tensor
          const variance = builder.input('variance', {dataType, dimensions});
          assert_throws_dom('DataError', () => builder.batchNormalization(input, mean, variance));
        }
      }
    }
...
promise_test(async t => {
  for (let dataType of operandDataTypeArray) {
    ...
    for (let axis of validAxisArray) {
      ...
      for (let offset of adjustOffsetArray) {
        const adjustedSize = size + offset;
        const variance = builder.input('variance', {dataType, dimensions: [adjustedSize]});
        assert_throws_dom('DataError', () => builder.batchNormalization(input, mean, variance, {axis}));
      }
    }
  }
