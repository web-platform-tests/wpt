...
# Function to return a ogg video and
# to apply CORS response header according
# to the GET parameters
#
def main(request, response):
    cors(request, response)
    # Set content type, this file only returns ogg videos
    response.headers.set("Content-type", "video/ogg")

    video = ""

    with open("/html/semantics/embedded-content/resources/movie.ogg", "rb") as f:
        video = f.readlines()
