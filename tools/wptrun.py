...
def args_general(kwargs):
    kwargs.set_if_none("tests_root", here)
    kwargs.set_if_none("metadata_root", here)
...
def setup_firefox(venv, kwargs, prompt=True):
    ...
    venv.install_requirements(os.path.join(here, "tools", "wptrunner", firefox.requirements))
...
def setup_chrome(venv, kwargs, prompt=True):
    ...
    venv.install_requirements(os.path.join(here, "tools", "wptrunner", chrome.requirements))
...
def setup_wptrunner(venv, product, tests, wptrunner_args, prompt=True,):
    ...
    wptrunner_path = os.path.join(here, "tools", "wptrunner")
...
def main():
    ...
    venv = virtualenv.Virtualenv(os.path.join(here, "_venv"))
    venv.start()
