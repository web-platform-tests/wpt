...
from ..wptrunner import get_pause_after_test, get_loader
...
def get_loader_with_fakes(
    ...
    spec_dir.mkdir()
    test_filenames = [f"test-{i:03d}.html" for i in range(0, 10)]
...
def test_get_loader_include(tmp_path):
...
def test_get_loader_exclude(tmp_path):
    _, test_loader = get_loader_with_fakes(
        tmp_path,
...
def test_get_loader_include_exclude(tmp_path):
    _, test_loader = get_loader_with_fakes(
        tmp_path,
        include=["/fake-spec/test-007.html", "/fake-spec/test-008.html"],
        exclude=["/fake-spec/test-007.html"],
...
def test_get_loader_include_file(tmp_path):
    include = ["/fake-spec/test-007.html", "/fake-spec/test-008.html"]
...
def test_get_loader_exclude_file(tmp_path):
    exclude = ["/fake-spec/test-007.html"]
...
def test_get_loader_include_exclude_file(tmp_path):
    include = ["/fake-spec/test-007.html", "/fake-spec/test-008.html"]
    exclude = ["/fake-spec/test-007.html"]
