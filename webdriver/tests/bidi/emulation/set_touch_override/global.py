...
async def test_top_level(bidi_session, create_user_context,
        get_max_touch_points, affected_user_context, initial_max_touch_points):
    affected_context = await bidi_session.browsing_context.create(
        type_hint="tab", user_context=affected_user_context)

    await bidi_session.emulation.set_touch_override(max_touch_points=MAX_TOUCHES_GLOBAL)
    ...
    another_affected_context = await bidi_session.browsing_context.create(
        type_hint="tab", user_context=affected_user_context)
    assert await get_max_touch_points(another_affected_context) == MAX_TOUCHES_GLOBAL
...
async def test_iframe(bidi_session, url, get_max_touch_points,
