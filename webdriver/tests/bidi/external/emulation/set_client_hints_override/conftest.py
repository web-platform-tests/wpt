    ...
    async def assert_navigation_client_hints(context, expected_hints):
                ...
                if isinstance(expected_value, bool):
                     ...
                     if isinstance(expected_value, str):
                         ...
                         for item in expected_value:
                             parts.append(f'"{item["brand"]}";v="{item["version"]}"')
        ...
        if expected_hints:
            ...
            if "platform" in expected_hints:
                check_header("Sec-CH-UA-Platform", f'"{expected_hints["platform"]}"')
            if "platform" in expected_hints:
                check_header("Sec-CH-UA-Platform", f'"{expected_hints["platform"]}"')
...
@pytest_asyncio.fixture
async def assert_client_hints(get_navigator_client_hints, assert_navigation_client_hints):
        ...
        if expected_hints is not None:
             if actual_js_hints is not None:
                 for key, value in expected_hints.items():
                     assert actual_js_hints[key] == value, f"JS property {key} mismatch"
