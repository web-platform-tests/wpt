    ...
    for (testname, expected) in tests:
        ...
        if(script_file == "js.py"):
            content += """
                ...
                {}
                ee.src = '{}/html/semantics/scripting-1/the-script-element/resources/js.py?script_name=embedded_f_test_{}';
            ...
            content +="""
                var ee = document.createElement('script');
                ee.src = '{}/html/semantics/scripting-1/the-script-element/resources/js_script.py?operation={}&description=SCRIPT - {} {} to {} {} - {} - {}';
                document.body.appendChild(ee);
            }}
            """.format(
                        origins["A"] if(settings["from_domain"] == settings["to_domain"]) else origins["B"],
