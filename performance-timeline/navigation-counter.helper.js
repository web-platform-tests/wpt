...
function runNavigationCounterTest(params, description) {
  const defaultParams = {
    ...
    funcBeforeNavigation: (constants) => {
      ...
      return window.performance.getEntriesByName(constants.performanceMarkName)[0].navigationCount;
    },
    funcAfterBFCacheLoad: (expectedNavigationCount, constants) => {
      window.performance.mark(constants.performanceMarkName + expectedNavigationCount);
      window.performance.measure(constants.performanceMeasureName + expectedNavigationCount, constants.performanceMarkName, constants.performanceMarkName + expectedNavigationCount);
      return [window.performance.getEntriesByName(constants.performanceMarkName + expectedNavigationCount)[0].navigationCount, window.performance.getEntriesByName(constants.performanceMeasureName + expectedNavigationCount)[0].navigationCount];
    },
  }
...
function runBfcacheWithMultipleNavigationTest(params, description) {
  ...
  promise_test(async t => {
    ...
    let navigationCount = await pageA.execute_script(params.funcBeforeNavigation, [params.constants])
    ...
    for (i = 0; i < params.navigationTimes; i++) {
      ...
      let navigationCounts = await pageA.execute_script(params.funcAfterBFCacheLoad, [i + 1, params.constants]);
      assert_implements_optional(navigationCounts.every(t => t === (i + 1)), "NavigationCount should all be " + (i + 1) + ".");
    }
  }, description);
}
