...
if TYPE_CHECKING:
    ...
    from tools.manifest import manifest  # noqa: TC004
    from tools.manifest.download import download_from_github  # noqa: TC004
...
ManifestItemsFilter: TypeAlias = "Callable[[Iterable[ManifestItertypesItem]], Iterable[ManifestItertypesItem]]"
...
class Subsuite:
    ...
    def manifest_filters(self, manifests: TestManifests) -> Sequence[ManifestItemsFilter]:
        if self.name:
...
class TestLoader:
    ...
    def __init__(
        ...
        manifest_filters: Sequence[ManifestItemsFilter] | None = None,
    ...
    def iter_tests(
        self, run_info: Mapping[str, object], manifest_filters: Iterable[ManifestItemsFilter]
...
class TestGroup:
    ...
    @property
    def name(self) -> str:
        return f"{self.subsuite}:{self.metadata.scope}" if self.subsuite else "/"
