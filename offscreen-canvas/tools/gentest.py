# This script is modified based on 2dcontext/tools/gentest.py
# Run this script to generate tests for OffscreenCanvas and its 2d context.
...
TESTOUTPUTDIR = '../../offscreen-canvas'
IMAGEOUTPUTDIR = '../../offscreen-canvas'
...
spec_assertions = []
for s in yaml.load(open('../../2dcontext/tools/spec.yaml', "r").read())['assertions']:
    if 'meta' in s:
...
tests = []
for t in sum([ yaml.load(open(f, "r").read()) for f in ['tests2d.yaml']], []):
    if 'DISABLED' in t:
        continue
    if 'meta' in t:
...
def write_annotated_spec():
    ...
    doc = xml.dom.minidom.parse(open('../../2dcontext/tools/current-work-canvas.xhtml', 'r'))
