...
promise_test(async (t) => {
  ...
  // 2. `finalUrl` is NOT intercepted by SW and receives Critical-CH header.
  ...
  await ch_sw_test(t,
    ...
    'critical-ch/None', // not intercepting `finalUrl`
    initialUrl,
    'PASS');
  const count = await (await fetch(`resources/redirect.py?key=${key}&action=get`)).text();
  assert_equals(count, "2");
}, "Critical-CH restarts the redirect chain from the beginning (not intercepted by SW)");
