...
# input: first argument should be directory in css1/ to process
...
die "$0: syntax: make-proper-tests.pl FILENAMES, e.g. 'make-proper-tests.pl css1/css1tests/FILENAMES'\n" if @ARGV != 1;
my $filenames = $ARGV[0];
die "$0: $filenames doesn't exist or is not a file\n" unless -f $filenames;
unless ($filenames =~ m/^(css1\/.+)\/FILENAMES$/os) {
    die "$0: must be invoked from source/ directory, on tests in the css1 directory, with file called FILENAMES\n";
}
my $directory = $1;
...
while (defined($_ = <FILENAMES>)) {
    ...
    my $output = "tests/css2_1/$filename";
...
system("cp -Lrvu $directory/support tests/css2_1");
