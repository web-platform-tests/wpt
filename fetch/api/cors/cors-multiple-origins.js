...
function corsMultipleOrigins(desc, originList, shouldPass) {
  var origins = ""
  var first = true;
  for (var idx in originList) {
    if (first)
      first = false;
    else
      origins += ", ";
    origins += originList[idx];
  }
  var urlParameters = "?origin=" + encodeURIComponent(origins);
...
corsMultipleOrigins("3 origins allowed, match twice (\"*\" and " + origin + ")", ["*", "http://example.com", "*"], true);
