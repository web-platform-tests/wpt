...
async_test(t => {
  ...
  // Step 2
  const id = Date.now();
  const relay_worker = new SharedWorker("/storage-access-api/resources/shared-worker-relay.js", id);
  relay_worker.port.onmessage = t.step_func(e => {
    ...
    // Step 8
    const count_worker = new SharedWorker("/storage-access-api/resources/shared-worker-count.js", id);
