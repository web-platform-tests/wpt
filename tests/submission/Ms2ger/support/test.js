...
var Test = {
  ...
  "isList" : function isList(aGot, aExpected, aMsg) {
    if (!this.is(aGot.length, aExpected.length,
                 "Should have the same length: got " + aGot.length +
                 ", expected " + aExpected.length +
                 ". Will not compare the elements.")) {
      return false;
    }
    for (var i = 0, il = aExpected.length; i < il; ++i) {
      if (!Test.is(aGot[i],
                   aExpected[i],
                   "Should have the same element at index " + i + ": got " +
                   aGot[i] + ", expected " + aExpected[i])) {
        return false;
      }
    }
    return true;
