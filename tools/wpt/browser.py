...
class Firefox(Browser):
    ...
    def extract_version_number(self, version_string: str):
    ...
    def get_version_and_channel_application_ini(self, path: str):
        ...
        if channel == "nightly":
            ...
            if source_repo is not None and commit is not None:
                if source_repo.startswith("https://hg.mozilla.org"):
                    try:
                        commit_data = get(f"https://hg-edge.mozilla.org/integration/autoland/json-rev/{commit}").json()
    ...
    def get_git_ref(self, version: str, channel: str, rev: Optional[str]):
        if rev is not None:
            return rev
        ...
        if channel == "beta":
            ...
            for tag_data in get(f"https://api.github.com/repos/mozilla-firefox/firefox/git/matching-refs/tags/{ref_prefix}").json():
    ...
    def get_profile_github(self, version: str, channel: str, dest: str, rev: Optional[str]):
