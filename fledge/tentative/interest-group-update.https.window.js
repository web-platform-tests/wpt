...
//                        This group will have a high bid to ensure if there was ever a tie, it would win.
...
// A possible case that can occur, with the current structure of the Protected Audience API is
// where the `successUpdateGroup` updates, and so does the `failedUpdateGroup`, but the `failedUpdateGroup`
// update happens significantly after the  `successUpdateGroup`'s update. In an effort to combat this,
// after the while loop we run another auction to ensure there is no winner (both cases should throw),
// but depending how slow the update takes, this flaky issue still can **possibly** occur.
