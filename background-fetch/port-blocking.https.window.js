...
}, 'fetch to default https port should register ok', 'resources/sw.js');

backgroundFetchTest((t, bgFetch) => {
  return bgFetch.fetch(uniqueId(), 'http://127.0.0.1');
}, 'fetch to default http port should register ok', 'resources/sw.js');

backgroundFetchTest((t, bgFetch) => {
  return bgFetch.fetch(uniqueId(), 'https://example.com:443');
}, 'fetch to port 443 should register ok', 'resources/sw.js');

backgroundFetchTest((t, bgFetch) => {
  return bgFetch.fetch(uniqueId(), 'https://example.com:80');
}, 'fetch to port 80 should register ok, even over https', 'resources/sw.js');
...
}, 'fetch to bad port (SMTP) should reject', 'resources/sw.js');
