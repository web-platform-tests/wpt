...
promise_test(async t => {
  ...
  assert_equals(logs.length, 3, 'Browser must follow redirect and fetch manifest via relative Link header');
  assert_equals(logs[0].endpoint, 'payment-method-identifier', 'First request must hit initial PMI URL');
  assert_equals(logs[0].method, 'HEAD', 'First request must use HEAD method');

  assert_equals(logs[1].endpoint, 'payment-method-identifier', 'Second request must hit post-redirect target PMI URL');
