...
def test_execute_script_handle_prompt_accept(new_session, add_browser_capabilites):
...
def test_execute_script_handle_prompt_dismiss(new_session, add_browser_capabilites):
...
def test_execute_script_handle_prompt_dismiss_and_notify(new_session, add_browser_capabilites):
...
def test_execute_script_handle_prompt_accept_and_notify(new_session, add_browser_capabilites):
...
def test_execute_script_handle_prompt_ignore(new_session, add_browser_capabilites):
...
def test_execute_script_handle_prompt_default(new_session, add_browser_capabilites):
...
def test_execute_script_handle_prompt_twice(new_session, add_browser_capabilites):
    _, session = new_session({"capabilities": {"alwaysMatch": add_browser_capabilites({"unhandledPromptBehavior": "accept"})}})
